...
public class Lab10P5Wrapper {
	...
	public static class HashTableSC<K, V> implements Map<K, V> {
		...
		public void put(K key, V value) {
			...
			double n = this.size();
			double N = buckets.length;
			if(n/N > loadFactor) {
				System.out.println("REHASHING NEEDED"); //ADD THIS BEFORE CALLING rehash(), TESTS WILL FAIL
				rehash();
			}
		...
		private void rehash() {
			...
			List<BucketNode<K, V>>[] rehashBuckets = new SinglyLinkedList[currentSize*2];

			for (int i = 0; i < rehashBuckets.length; i++)
				rehashBuckets[i] = new SinglyLinkedList<BucketNode<K, V>>();

			for(List<BucketNode<K,V>> list : buckets) {
				for (int i = 0; i < list.size(); i++) {
					BucketNode<K,V> newBucket = list.get(i);
					int newIndex = hashFunction.hashCode(newBucket.getKey()) % rehashBuckets.length;

					rehashBuckets[newIndex].add(newBucket);
				}
			}
	...
	public static class HashSet<E> implements Set<E> {
		...
		@Override
		public Set<E> union(Set<E> S2) {
			Set<E> S3 = new HashSet<>(this.size() + S2.size());

			for(E e : hashtable.getKeys()){
				S3.add(e);
			}
		...
		@Override
		public boolean isSubSet(Set<E> S2) { 
			return this.intersection(S2).size() != 0;
		}
