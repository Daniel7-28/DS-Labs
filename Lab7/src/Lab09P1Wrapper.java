public class Lab09P1Wrapper {
	...
	public static class BinaryTree<E> implements Tree<E> {
		...
		public int getLevelCount(int level) {
			if(level <= 0)
				throw new IllegalArgumentException("level must be al least 1");
		...
		private int getRecLevelCount(BinaryTreeNode<E> root, int level) {
			...
			if(level == 1) return 1;
			else return getRecLevelCount(root.getLeftChild(), level-1) + getRecLevelCount(root.getRightChild(), level-1);	
		}
	}
