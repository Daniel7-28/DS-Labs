...
public class P3Wrapper {
	...
	public static class BinarySearchTree<K, V> implements Map<K, V> {
		...
		public ArrayList<KeyValuePair<K, V>> inRangeValues(K key1, K key2) {
			...
			inRangeValuesAux(key1, key2, L, this.root);	

			return L;
		}

		public void inRangeValuesAux(K key1, K key2, ArrayList<KeyValuePair<K, V>> L, BinaryTreeNode<MapEntry<K, V>> N){
			if(N == null) 
				return;

			int comparison1 = this.keyComparator.compare(N.getValue().getKey(),key1);
			int comparison2 = this.keyComparator.compare(N.getValue().getKey(), key2);

			if (comparison1 >= 0 && comparison2 < 0) {
				MapEntry<K,V> M = new MapEntry<K,V>(N.getValue().getKey(), N.getValue().getValue());
				L.add(M);
			}
			inRangeValuesAux(key1,key2, L,N.getLeftChild());
			inRangeValuesAux(key1,key2, L,N.getRightChild());
		}
	}
}
